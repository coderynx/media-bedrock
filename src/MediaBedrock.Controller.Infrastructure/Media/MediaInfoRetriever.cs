...
{
    /// <inheritdoc />
    public async Task<Result<MediaInformation>> RetrieveAsync(string uri)
    {
        // TODO: Move this into a more generic location.
        if (!File.Exists(uri))
        {
            logger.LogError("File not found at {Uri}", uri);
            return MediaErrors.ObjectNotFound(uri);
        }
        ...
        if (exitCode is not 0)
        {
            ...
            return MediaErrors.RetrievalFailed(uri);
        }
